...
def main():
    ...
    for slug in SLUGS:
        ...
        sankalpana_meta.append({'slug': slug, 'title': data['title']})
    ...
    # Write js/sankalpana-list.js — {slug, title} manifest used by js/notify.js
    # for the daily notification rotation. Order matches SLUGS (stable day-index cycling).
    entries = ',\n  '.join(
        '{ slug: %s, title: %s }' % (json.dumps(m['slug']), json.dumps(m['title'], ensure_ascii=False))
        for m in sankalpana_meta
    )
    list_js_path = Path("js/sankalpana-list.js")
    list_js_path.write_text(
        "// Auto-generated by build_sankalpana.py — do not edit by hand.\n"
        "window.SANKALPANA_LIST = [\n  " + entries + "\n];\n",
        encoding='utf-8'
    )
    print(f"Wrote {list_js_path} ({len(sankalpana_meta)} entries)")
